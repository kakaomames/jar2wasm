...
# -------------------------------------------------------------
echo "=== Step 3-1: Modifying \${JSON_FILE} via jq ==="

# 例: バックアップを作っておく
cp "\(JSON_FILE" "\){JSON_FILE}.bak"

# ※もし「特定のライブラリを削りたい」「引数を書き換えたい」場合は、
# ここで `jq '.arguments.game += ["--demo"]' $JSON_FILE > tmp.json && mv tmp.json $JSON_FILE` 
# のように、jq を使って綺麗に中身を書き換えるハックを仕込めます。
...
# jq で「.libraries[] の中の downloads.artifact.url」を狙い撃ちし、
# ヌル値や中身が空のものを排除して綺麗なリストにしてループに流す
